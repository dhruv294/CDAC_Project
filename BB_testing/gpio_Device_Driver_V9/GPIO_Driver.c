...
static int __init DESDGPIO_init(void){

	int result;
	int i;
	printk(KERN_INFO "%s: GPIO Driver inserted\n", THIS_MODULE->name);
	ledOn = 0;
	...
	devno = MKDEV(major,0);
	if((s_pDeviceObject = device_create(s_pDeviceClass, NULL, devno, NULL, "GPIOOUT")) == NULL)
	{
	...
	devno = MKDEV(major,1);
	if((device_create(s_pDeviceClass, NULL, devno, NULL, "GPIOIN")) == NULL)
	{
	...
	irqNumber = gpio_to_irq(gpioButton);
	printk(KERN_INFO "%s: The button is mapped to IRQ: %d\n", THIS_MODULE->name,irqNumber);
	result = request_irq(irqNumber,(irq_handler_t) gpio_irq_handler,IRQF_TRIGGER_RISING,"gpio_handler",NULL);
...
module_init(DESDGPIO_init);
module_exit(DESDGPIO_exit);

MODULE_LICENSE("GPL");
MODULE_AUTHOR("Dhruv Sahani <dhruvshn@gmail.com>, Bipin Dangwal <mr.bipindangwal@gmail.com>");
MODULE_DESCRIPTION("A simple Linux GPIO driver for the BBB");
MODULE_VERSION("1");
